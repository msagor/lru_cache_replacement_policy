...
int *leader = new int[128];  			//leader array
...
int leader_set_access =0;				//a counter to count leader set access..when it turns 128, it decides which policy worked the best
int policy_1_hit=0;
int policy_2_hit=0;
//-----DO NOT CHANGE THESE VARIABLE ASSIGNMENTS-----
...
void InitReplacementState(){	
	...
	if(leader_array_populating==1){             //populates the leader array with random number between 1 and 1024
        ...
        for(int i =0;i<128;i++){                 //initialize the leader array with all -1s initially
            leader[i]= -1;
        }
		
	for(int i =0;i<128;i++){						//initialize the leader array with distinctive random numbers
            distinctive = 0;
            while(distinctive == 0){
